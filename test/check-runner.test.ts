...
describe('runPostResolveCheck', () => {
  it('reports a passing command', async () => {
    const result = await runPostResolveCheck('node -e "process.exit(0)"', process.cwd(), process.env, false);
  ...
  it('reports a failing exit code and captures output', async () => {
    const result = await runPostResolveCheck('node -e "console.log(\'boom\'); process.exit(2)"', process.cwd(), process.env, true);
  ...
  it('reports a timeout, killing the process and capping duration', async () => {
    ...
    const result = await runPostResolveCheck('node -e "setTimeout(() => {}, 5000)"', process.cwd(), process.env, false, undefined, 50);
    expect(result.ok).toBe(false);
  ...
  it('caps the output tail at 4,000 characters', async () => {
    const result = await runPostResolveCheck('node -e "process.stdout.write(\'x\'.repeat(5000))"', process.cwd(), process.env, false);
  ...
  it('reports cancellation via an abort signal without treating it as a timeout', async () => {
    ...
    const promise = runPostResolveCheck('node -e "setTimeout(() => {}, 5000)"', process.cwd(), process.env, false, controller.signal);
    controller.abort();
